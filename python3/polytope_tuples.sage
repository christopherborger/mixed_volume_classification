...
def mixed_volume(T):
    ...
    T = np.array(T)
    mv = 0
    for i in it.product([True,False],repeat=d):
        i = list(i)
        pos_entries = sum(i)
        if pos_entries != 0:
            mv += (-1)**(d-pos_entries) * list(map(sum,[T[i]]))[0].volume()
    return mv
...
def Cayley(P):
    ...
    """
    k=len(P)
    A=list(map(tuple,[zero_vector(QQ,k-1)]+identity_matrix(k-1).rows()))
    V=[]
    for i in range(k):
...
def save_polytope_tuples(tuples,fname):
    f=open(fname,'w')
    print([[list(map(tuple,P.vertices())) for P in t] for t in tuples], file=f)
    f.close()

def polytope_tuples_from_file(fname):
    f=open(fname,'r')
    L=eval(f.read().replace('\n',' '))
    L=[list(map(Polyhedron,t)) for t in L]
    return L
